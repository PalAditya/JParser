...
public class LR0Parser extends Parser
{
    ...
    public void buildDFA()
    {
        augment();
        dfa.id=getId();
        rules.entrySet().forEach((mp) -> {
            mp.getValue().forEach((al) -> {
    ...
    /**
     * Generates the parsing table and returns whether the grammar is LR0 or not
     * @return boolean
     * @param output: Option to switch on/off the parsing table display 
     *@since 0.1
     */
    public boolean getParsingTable(boolean output)
    {
        ...
        for(DFA dfa:states)
        {
            for(Map.Entry<String,DFA> mp:dfa.transitions.entrySet())
            {
                ...
                {                    
                    if (!states.get(index).rules.isEmpty())
                ...
                if(table[i-1][j-1].size()>1)
                {
                    ...
                    for(String string:table[i-1][j-1])
                    {
                        if(string.contains("Shift"))
                            shift++;
                        else if(string.contains("Reduce"))
                            reduce++;
                    }
